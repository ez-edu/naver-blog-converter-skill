...
description: Convert Naver Blog posts or drafts into Tistory- and WordPress-ready Markdown/HTML with GEO/AEO optimization, source attribution, headings, links, and image placeholders.
---
...
- add GEO/AEO optimization for answer engines and generative search:
  - direct answer summary for the first screen;
  - SEO title and meta description suggestions;
  - question-style H2/H3/FAQ blocks;
  - keyword/entity suggestions;
  - Article/FAQPage JSON-LD draft for WordPress SEO plugins;
...
- tone: faithful migration, SEO rewrite, GEO/AEO optimized rewrite, casual rewrite, corporate blog rewrite;
- optimization mode: `both` by default, or `geo`, `aeo`, `none`;
- image policy:
...
--optimize-for none|geo|aeo|both
                              Add GEO/AEO summary, FAQ, metadata, and JSON-LD guidance. Default: both
...
5. Apply GEO/AEO optimization unless the user requests faithful/no-optimization output.
   - GEO means optimizing the converted draft so generative engines can identify the topic, entities, answer, evidence, and FAQ structure.
   - AEO means optimizing for answer engines/snippets by placing a direct answer summary near the top and using question-style headings.
   - Keep claims faithful to the source; do not invent facts, statistics, prices, reviews, credentials, or product claims.
   - Add a concise `핵심 요약` section and `자주 묻는 질문` section to the draft.
   - Generate sidecar optimization files with SEO title, meta description, primary/secondary keywords, FAQ, checklist, and Article/FAQPage JSON-LD draft.
   - Treat JSON-LD as a draft for review; do not paste structured data into a live site without user review.
